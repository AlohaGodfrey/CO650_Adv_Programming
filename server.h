...
inline bool Server::sendMsg()
{
	...
	int ClientDesc = send(clientSocket, getClientMsg().c_str(), getClientMsg().size() + 1, 0);
...
inline bool Server::recieveMsg()
{
	...
	int RecSize = recv(clientSocket, DataValue, 2000, 0);

	//data exception handling code
